...
const GhumakariKalakar = () => {
  ...
  const {
    ...
    queryKey: ["ghumakari-kalakar-events", eventTypeId, selectedCity, selectedDate],
    queryFn: () => {
        ...
        <div className="container-padding py-8">
          <div className="text-center mb-12">
            ...
            <p className="max-w-2xl mx-auto" style={{ color: '#F7D774' }}>
              Discover and book the best upcoming Ghumakari Kalakar experiences in your city. Each event type has its own vibe and curated experience, all organized by date for easy browsing.
            </p>
          </div>
          {/* Filters Section */}
          <div className="flex flex-col md:flex-row gap-4 items-center justify-center mb-8">
            <div className="w-full md:w-64">
              <Select value={selectedCity} onValueChange={setSelectedCity}>
                <SelectTrigger className="rounded-2xl font-medium border-none focus:ring-2 focus:ring-[#1B9AC6]" style={{ background: "#F7D774", color: "#FF6C0E" }}>
                  <SelectValue placeholder="Filter by city" />
                </SelectTrigger>
                <SelectContent className="bg-[#F7D774] text-[#FF6C0E]">
                  {cities.map((city) => (
                    <SelectItem key={city} value={city} className="text-[#FF6C0E]">
                      {city}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <div className="w-full md:w-64">
              <Select value={selectedDate} onValueChange={setSelectedDate}>
                ...
                <SelectContent className="bg-[#F7D774] text-[#FF6C0E]">
                  {dates.map((date) => (
                    <SelectItem key={date} value={date} className="text-[#FF6C0E]">
                      {date}
                    </SelectItem>
                  ))}
                </SelectContent>
                  ...
                  <div className="rounded-3xl px-8 py-8" style={{ background: '#F7D774' }}>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {dateEvents.map((event, index) => (
                        <Card key={event.id} className="hover-scale border-none shadow-soft overflow-hidden" style={{ backgroundColor: '#FF6C0E' }}>
